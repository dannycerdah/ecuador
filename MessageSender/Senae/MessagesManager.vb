...
#Region "IIE"
    ...
    Public Shared Function SendIIEDoc(DAE As String, Piezas As Integer, Peso As Double, Producto As String) As String
        ...
        Try
            If DAE = String.Empty Then
                ...
                Exit Function
            End If
            Result = ExecuteIIE(BuildSoap(DAE, Piezas, Peso, Producto))
        Catch ex As Exception
            'FileIO.FileSystem.WriteAllText("C:\xml\log.txt", Date.Now.ToString("yyyy-MM-dd HH:mm:ss") & " - " & ex.Message & ControlChars.CrLf & ControlChars.CrLf & ControlChars.CrLf, True)
            FileIO.FileSystem.WriteAllText(rutaejecutable & "\xml\log.txt", Date.Now.ToString("yyyy-MM-dd HH:mm:ss") & " - " & ex.Message & ControlChars.CrLf & ControlChars.CrLf & ControlChars.CrLf, True)
        End Try
    ...
    Private Shared Function BuildSoap(DAE As String, Piezas As Integer, Peso As Double, Producto As String) As String
        ...
        Try
            ...
            xml = xml.Replace("http://schemas.microsoft.com/clr/nsassem/TestISenae/TestISenae%2C%20Version%3D1.0.0.0%2C%20Culture%3Dneutral%2C%20PublicKeyToken%3Dnull", "http://soapinterop.org/xsd")
    ...
    Private Shared Function ExecuteIIE(xml As String) As String
        Dim result As String = String.Empty
        Dim rutaejecutable As String = Application.StartupPath
        Dim carpeta As String = Date.Now.ToString("yyyy-MM-dd")
        If Not FileIO.FileSystem.DirectoryExists(rutaejecutable & "\xml") Then
            FileIO.FileSystem.CreateDirectory(rutaejecutable & "\xml")
        End If
        If Not FileIO.FileSystem.DirectoryExists(rutaejecutable & "\xml\IIE\" & carpeta) Then
            FileIO.FileSystem.CreateDirectory(rutaejecutable & "\xml\IIE\" & carpeta)
        End If
        Try
            Dim objRes As XmlDocument
            Dim ID, Anio, Sec As String
            Anio = Date.Now.Year
            Sec = My.Settings.Secuential
            Sec = Sec.PadLeft(8, "0")
            ID = "29000001" & Anio & Sec & "S"
            'MARZ_15.08.17 
            'Dim carpeta As String = Date.Now.ToString("yyyy-MM-dd")
            'If Not FileIO.FileSystem.DirectoryExists("C:\xml\IIE\" & carpeta & "\") Then
            '    FileIO.FileSystem.CreateDirectory("C:\xml\IIE\" & carpeta & "\")
            'End If
            'End MARZ
            'FileIO.FileSystem.WriteAllText("C:\xml\IIE\" & carpeta & "\REQ" & ID & ".xml", xml, True) 'MARZ
            FileIO.FileSystem.WriteAllText(rutaejecutable & "\xml\IIE\" & carpeta & "\REQ" & ID & ".xml", xml, True)
            objRes = CallWebService("http://cdes.aduana.gob.ec/cdes_svr/SENAE_ExportCargaService?wsdl", xml)
            'objRes.Save("C:\xml\IIE\" & carpeta & "\RES" & ID & ".xml") 'MARZ
            objRes.Save(rutaejecutable & "\xml\IIE\" & carpeta & "\RES" & ID & ".xml") 'MARZ
...
#Region "DAE_SRCHEX"
    ...
    Public Shared Function SearchDAE(StartDate As Date, EndDate As Date) As DataTable 'Transacción 
        Return BulkSearchDAE(StartDate, EndDate, String.Empty)
    End Function

    Public Shared Function SearchDAE(DAE As String) As DataTable 'Consulta
    ...
    Private Shared Function SRCHEX(xml As String) As DataTable
        ...
        If Not FileIO.FileSystem.DirectoryExists(rutaejecutable & "\xml") Then
            FileIO.FileSystem.CreateDirectory(rutaejecutable & "\xml")
        End If
        If Not FileIO.FileSystem.DirectoryExists(rutaejecutable & "\xml\DAE\" & carpeta) Then
            FileIO.FileSystem.CreateDirectory(rutaejecutable & "\xml\DAE\" & carpeta)
        End If
        'If Not FileIO.FileSystem.DirectoryExists("C:\xml\DAE\" & carpeta & "\") Then
        '    FileIO.FileSystem.CreateDirectory("C:\xml\DAE\" & carpeta & "\")
        'End If
        'End MARZ
        Try
            Dim ID As String = Date.Now.Ticks
            'If My.Settings.KeepDaeLog Then FileIO.FileSystem.WriteAllText("C:\xml\DAE\" & carpeta & "\REQ" & ID & ".xml", xml, True) 'MARZ
            If My.Settings.KeepDaeLog Then FileIO.FileSystem.WriteAllText(rutaejecutable & "\xml\DAE\" & carpeta & "\REQ" & ID & ".xml", xml, True) 'MARZ
            ...
            If My.Settings.KeepDaeLog Then objRes.Save(rutaejecutable & "\xml\DAE\" & carpeta & "\RES" & ID & ".xml") 'objRes.Save("C:\xml\DAE\" & carpeta & "\RES" & ID & ".xml") 'MARZ
            ...
            Join c In ds.Tables("StatusDeclaration").AsEnumerable On a.Field(Of Integer)("Declaration_Id") Equals c.Field(Of Integer)("Declaration_Id")
            Select a, b, c
            Dim rows As DataRow()
            For Each obj In query
                ...
                newRow.Item("EffectiveEndDate") = obj.a.Item("EffectiveEndDate")
                newRow.Item("response_Id") = 0
                newRow.Item("examinationChannelName") = obj.b.Item("examinationChannelName")
                newRow.Item("StatusName") = obj.c.Item("StatusName")
...
#Region "DAS_SRCHEX"
    ...
    Public Shared Function SearchDAS(StartDate As Date, EndDate As Date) As DataTable
        Return BulkSearchDASEX(StartDate, EndDate, String.Empty)
    End Function

    Public Shared Function SearchDAS(DAS As String) As DataTable
        Return BulkSearchDASEX(Date.Now, Date.Now, DAS)
    End Function

    Private Shared Function BulkSearchDASEX(StartDate As Date, EndDate As Date, DAE As String) As DataTable
        Dim xml As String = String.Empty
        Dim result As New DataTable
        Try
            If DateDiff(DateInterval.Day, StartDate, EndDate) > 14 Then
            ...
            End If
            Dim dataEcuapass() As String = Ecuapass.getDataEcuapass() 'MARZ
            xml = My.Settings.DASEX_req 'Cabecera de seguridad
            'MARZ
            xml = xml.Replace("usuarioEcuapass", dataEcuapass(0))
            xml = xml.Replace("claveEcuapass", dataEcuapass(1))
            xml = xml.Replace("keyEcuapass", dataEcuapass(2))
            'MARZ - End
            If DAE = String.Empty Then
                xml = xml.Replace("2015-07-28", StartDate.ToString("yyyy-MM-dd"))
                xml = xml.Replace("2015-07-30", EndDate.ToString("yyyy-MM-dd"))
            Else
                xml = xml.Replace("DeclarationNumberID&gt;&lt;/", "DeclarationNumberID&gt;" & DAE & "&lt;/")
                xml = xml.Replace("2015-07-28", "")
                xml = xml.Replace("2015-07-30", "")
                xml = xml.Replace("&lt;ConsultTypeID&gt;02&lt;/ConsultTypeID&gt;", "&lt;ConsultTypeID&gt;03&lt;/ConsultTypeID&gt;")
            End If
            result = DASEX(xml)
        Catch ex As Exception
            FileIO.FileSystem.WriteAllText("D:\xml\log.txt", Date.Now.ToString("yyyy-MM-dd HH:mm:ss") & " - " & ex.Message & ControlChars.CrLf & ControlChars.CrLf & ControlChars.CrLf, True)
            Throw ex
        End Try
        Return result
    End Function

    Private Shared Function DASEX(xml As String) As DataTable
        Dim objRes As XmlDocument
        Dim newRes As DataTable = New DataTable("BulkDAE")
        Dim newRow As DataRow
        'MARZ_15.08.17 
        Dim carpeta As String = Date.Now.ToString("yyyy-MM-dd")
        Dim rutaejecutable As String = Application.StartupPath
        If Not FileIO.FileSystem.DirectoryExists(rutaejecutable & "\xml") Then
            FileIO.FileSystem.CreateDirectory(rutaejecutable & "\xml")
        End If
        If Not FileIO.FileSystem.DirectoryExists(rutaejecutable & "\xml\DAS\" & carpeta) Then
            FileIO.FileSystem.CreateDirectory(rutaejecutable & "\xml\DAS\" & carpeta)
        End If
        'If Not FileIO.FileSystem.DirectoryExists("C:\xml\DAS\" & carpeta & "\") Then
        '    FileIO.FileSystem.CreateDirectory("C:\xml\DAS\" & carpeta & "\")
        'End If
        'End MARZ
        Try
            Dim ID As String = Date.Now.Ticks
            'FileIO.FileSystem.WriteAllText("C:\xml\DAS\" & carpeta & "\REQ" & ID & ".xml", xml, True) 'MARZ
            FileIO.FileSystem.WriteAllText(rutaejecutable & "\xml\DAS\" & carpeta & "\REQ" & ID & ".xml", xml, True) 'MARZ
            objRes = CallWebService("http://cdes.aduana.gob.ec/cdes_svr/SENAE_ExportDespachoService_EC?wsdl", xml, "requestExportDespachoDASEX")
            'objRes.Save("C:\xml\DAS\" & carpeta & "\RES" & ID & ".xml") 'MARZ
            objRes.Save(rutaejecutable & "\xml\DAS\" & carpeta & "\RES" & ID & ".xml") 'MARZ
            With newRes
                .Columns.Add("DeclarationNumber", GetType(String))
                .Columns.Add("exporterDocumentId", GetType(String))
                .Columns.Add("exporterName", GetType(String))
                ...
                .Columns.Add("response_Id", GetType(String))
                .Columns.Add("examinationChannelName", GetType(String))
                .Columns.Add("GovernmentProcedureID", GetType(String))
                .Columns.Add("GovernmentProcedureName", GetType(String))
                .Columns.Add("RemNumber", GetType(String))
                .Columns.Add("RemRucAgency", GetType(String))
                .Columns.Add("RemEndVigency", GetType(String))
                '.Columns.Add("response_Id", GetType(String))
            ...
            Dim query =
            From a In ds.Tables("Declaration").AsEnumerable
            Join b In ds.Tables("ExaminationChannel").AsEnumerable On a.Field(Of Integer)("Declaration_Id") Equals b.Field(Of Integer)("Declaration_Id")
            Join c In ds.Tables("GovernmentProcedure").AsEnumerable On a.Field(Of Integer)("Declaration_Id") Equals c.Field(Of Integer)("Declaration_Id")
            Select a, b, c
            Dim rows As DataRow()
            For Each obj In query
                newRow = newRes.NewRow()
                ...
                newRow.Item("response_Id") = 0
                newRow.Item("examinationChannelName") = obj.b.Item("examinationChannelName")
                newRow.Item("GovernmentProcedureID") = obj.c.Item("GovernmentProcedureID")
                newRow.Item("GovernmentProcedureName") = obj.c.Item("GovernmentProcedureName")
                If Not IsNothing(ds.Tables("RemInfoDetail")) Then
                    rows = ds.Tables("RemInfoDetail").Select("Declaration_Id = " & obj.a.Item("Declaration_Id") & "")
                    If rows.Count > 0 Then
                        For Each r As DataRow In rows
...
#End Region
    ...
    Public Shared Function QueryDocStatus(eDocId As String, SubmitDate As Date) As String
        ...
        Try
            ...
            If My.Settings.KeepDocLog Then xRes.Save(rutaejecutable & "\xml\QRY\" & Date.Now.Ticks.ToString & ".xml") 'xRes.Save("C:\xml\QRY\" & Date.Now.Ticks.ToString & ".xml")
    ...
    Private Shared Function QueryDocumentResponseByReferenceId(FunctionalReferenceID As String, SubmitDate As Date) As DeclarationResponse
        ...
        Dim xml As String = My.Settings.DocumentResponse_req
        Dim ds As New DataSet
        Dim xRes As New XmlDocument
        ...
        Try
            ...
            xRes = CallWebService("http://cdes.aduana.gob.ec/cdes_svr/SENAE_CommonResponseService_EC?wsdl", xml, "requestNotificationAcceptanceNA")
